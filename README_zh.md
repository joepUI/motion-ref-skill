...
[Claude Code](https://docs.anthropic.com/en/docs/claude-code) 技能 — 现代 UI 动效参考库。112 种可直接使用的动画效果，13 个分类。用自然语言描述交互场景，匹配最合适的动效，输出 CSS/JS 代码。
...
| 3 | 弹层与浮层 | 10 | 弹窗、底部面板、Toast、Tooltip、抽屉、命令面板 |
| 4 | 导航与转场 | 8 | 页面推入、共享元素过渡、标签指示器、Dock 导航 |
| 5 | 表单与输入 | 10 | 聚焦环、浮动标签、开关、验证码输入、滑块、步骤指示器 |
| 6 | 文字与数据 | 10 | 打字机、数字滚动、翻牌计数、光泽文字、渐变文字 |
| 7 | 滚动驱动 | 5 | 滚动揭示、阅读进度条、滚动文字揭示 |
| 8 | 图标与标识 | 10 | 汉堡菜单、SVG 描边、角标弹跳、爱心点赞、铃铛抖动、复制打勾 |
| 9 | 轮播与列表 | 8 | 轮播、卡片堆叠、拖拽排序、手风琴、头像堆叠、滑动操作 |
| 10 | 状态切换 | 9 | 主题切换、空状态、在线状态、收藏星标、锁定解锁 |
| 11 | 氛围与背景 | 8 | 渐变流动、粒子、磁性光标、3D 卡片悬浮、流星雨 |
