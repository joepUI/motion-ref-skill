...
[Claude Code](https://docs.anthropic.com/en/docs/claude-code) 技能 — 现代 UI 动效参考库。105 种可直接使用的动画效果，13 个分类。用自然语言描述交互场景，匹配最合适的动效，输出 CSS/JS 代码。
...
| # | 分类 | 数量 | 代表效果 |
|---|------|------|---------|
| 1 | 加载与等待 | 9 | Spinner、骨架屏、环形进度、呼吸脉冲 |
| 2 | 入场与退场 | 8 | 淡入、上滑入场、弹性入场、序列入场、模糊入场 |
| 3 | 按钮与点击反馈 | 9 | 按压缩小、涟漪、成功打勾、错误抖动、庆祝粒子 |
| 4 | 弹层与浮层 | 10 | 弹窗、底部面板、Toast、Tooltip、抽屉、命令面板 |
| 5 | 导航与转场 | 8 | 页面推入、共享元素过渡、标签指示器、Dock 导航 |
| 6 | 表单与输入 | 10 | 聚焦环、浮动标签、开关、验证码输入、滑块、步骤指示器 |
| 7 | 文字与数据 | 10 | 打字机、数字滚动、翻牌计数、光泽文字、渐变文字 |
| 8 | 滚动驱动 | 7 | 滚动揭示、视差、阅读进度条、滚动吸附 |
| 9 | 图标与标识 | 7 | 汉堡菜单、SVG 描边、角标弹跳、图标变形 |
| 10 | 轮播与列表 | 8 | 轮播、卡片堆叠、拖拽排序、手风琴、头像堆叠、滑动操作 |
| 11 | 状态切换 | 6 | 主题切换、骨架到内容、错误恢复、庆祝动画 |
| 12 | 氛围与背景 | 8 | 渐变流动、粒子、磁性光标、3D 卡片悬浮、流星雨 |
| 13 | 数据可视化 | 5 | 柱状图升起、环形图、仪表盘、折线描绘、统计卡片 |

## 在线预览

浏览全部 105 种动效的在线演示：**https://joepui.github.io/motion-ref-skill/**
