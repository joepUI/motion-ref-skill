...
slug: motion-ref
version: 2.0.0
description: "现代 UI 动效参考库。105 种实用动效，13 个分类。用户描述场景，匹配最合适的方案，输出 CSS/JS 代码。触发词：动效推荐、用什么动效、什么动画好、推荐转场、加载动画、怎么让页面动起来、motion ref。"
---
...
| 意图 | 用户可能说的话 | 优先搜索的分类 |
|------|-------------|-------------|
| 等待/加载 | "loading""转圈""骨架屏""进度" | 1. 加载与等待 |
| 元素出现 | "出现""弹出""入场""淡入" | 2. 入场与退场 |
| 操作反馈 | "点击""按下""成功""失败""庆祝" | 3. 按钮与点击反馈 |
| 弹窗浮层 | "弹窗""下拉""菜单""toast""命令面板" | 4. 弹层与浮层 |
| 页面切换 | "转场""切换""跳转""返回""dock" | 5. 导航与转场 |
| 表单输入 | "输入""校验""开关""选中""OTP""滑块""步骤" | 6. 表单与输入 |
| 文字数字 | "打字""数字跳动""滚动文字""渐变文字" | 7. 文字与数据 |
| 滚动相关 | "滚动出现""视差""进度条""吸附" | 8. 滚动驱动 |
| 图标动画 | "图标切换""菜单变叉""badge""变形" | 9. 图标与标识 |
| 列表内容 | "轮播""列表增删""手风琴""滑动删除" | 10. 轮播与列表 |
| 状态变化 | "暗色模式""空状态""切换主题""庆祝" | 11. 状态切换 |
| 氛围装饰 | "高级感""科技感""背景纹理""3D" | 12. 氛围与背景 |
| 图表数据 | "图表""仪表盘""柱状图""折线图""统计" | 13. 数据可视化 |
...
#### 3.8 Fill Progress 填充进度
- **视觉**：按钮背景从左到右填充
- **场景**：长按确认、危险操作二次确认
- **CSS**：伪元素 `width: 0 → 100%` + transition

#### 3.9 Confetti Button 庆祝粒子
- **视觉**：点击后从按钮中心爆发彩色粒子
- **场景**：成功操作、庆祝反馈、游戏化 UI
...
#### 8.2 Parallax 视差滚动
- **视觉**：前后景不同速度移动
- **实现**：CSS `transform: translateY(calc(var(--scroll) * -0.3))` + JS 更新 `--scroll`

#### 8.3 Progress Indicator 阅读进度条
- **视觉**：页面顶部细条随滚动填充
- **实现**：JS `scrollY / (scrollHeight - innerHeight)` → 更新 `scaleX()`

#### 8.4 Sticky Shrink 吸顶缩小
- **视觉**：导航栏吸顶后缩小
- **CSS**：`position: sticky; transition: padding 250ms, font-size 250ms`

#### 8.5 Line Reveal 逐行高亮
- **视觉**：文字逐行从暗到亮
- **CSS**：每行 span + 递增 `animation-delay`

#### 8.6 Image Reveal 图片揭示
- **视觉**：clip-path 从一侧揭开图片
- **CSS**：`clip-path: inset(0 100% 0 0) → inset(0 0 0 0)` + transition

#### 8.7 Scroll Snap 滚动吸附
- **视觉**：滚动结束时自动对齐到最近的面板
- **CSS**：`scroll-snap-type: x mandatory` + 子元素 `scroll-snap-align: start`
- **要点**：隐藏滚动条 `scrollbar-width: none` + `::-webkit-scrollbar { display: none }`
...
#### 11.1 Theme Toggle 主题切换
- **视觉**：圆形扩散切换明暗主题
- **CSS**：`clip-path: circle()` 从点击位置扩散

#### 11.2 Empty State 空状态
- **视觉**：空状态图标浮动 + 淡入
- **CSS**：图标 `animation: float 3s ease-in-out infinite`

#### 11.3 Skeleton → Content 骨架到内容
- **视觉**：骨架层淡出，真实内容层淡入
- **CSS**：两层用 `grid-area: 1/1` 叠放 + `.loaded` 时切换 opacity

#### 11.4 Error → Recovery 错误恢复
- **视觉**：错误状态（红色）过渡到正常状态（绿色）
- **CSS**：`transition: all .35s ease` + 颜色和图标切换

#### 11.5 Expand Detail 展开详情
- **视觉**：摘要点击后展开为详情
- **CSS**：`grid-template-rows: 0fr → 1fr` + 圆角过渡

#### 11.6 Confetti Celebration 庆祝动画
- **视觉**：成功时从中心爆发彩色粒子/纸屑
- **场景**：支付成功、任务完成、游戏胜利
- **实现**：JS 动态创建 25+ 个粒子，随机角度/距离/旋转 + CSS `transform: translate(--tx, --ty) rotate(--rot)`
- **要点**：粒子混合圆形和方形，颜色用品牌色系
